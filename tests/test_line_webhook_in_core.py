...
def _line_tests_stub_chat_model_config(monkeypatch):
    """``lifespan`` calls ``ensure_chat_model_configured``; stub so tests do not depend on env / ~/.kumi.

    CI may set ``KUMI_CHAT_MODEL`` to an empty value or merge config in an order that still leaves
    ...
    monkeypatch.setattr("kumi.core.api.app_factory.ensure_chat_model_configured", _ensure)
...
def test_line_webhook_bad_signature(monkeypatch):
    monkeypatch.setenv("KUMI_LINE_INCORE", "1")
...
def test_line_webhook_text_message_single_user(monkeypatch):
    ...
    monkeypatch.setattr("kumi.line.handlers.stream_line_chat", _stream_one_text)
