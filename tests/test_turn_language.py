...
def test_detects_japanese_from_kana_even_with_cjk() -> None:
    ...
    assert "reply in Japanese" in build_turn_language_note("疲れた、めっちゃ眠い")


def test_detects_chinese_when_cjk_has_no_kana() -> None:
    assert detect_prompt_language("嗯嗯，我要睡觉了") == "Chinese"
    assert "reply in Chinese" in build_turn_language_note("嗯嗯，我要睡觉了")
...
def test_latin_script_prompts_keep_specific_language_to_model() -> None:
    ...
    assert detect_prompt_language("I'm exhausted and really sleepy") == "Latin-script language"
    assert "reply in that same natural language" in note
...
def test_ambiguous_prompt_still_says_not_to_follow_history() -> None:
    ...
    assert "not from earlier conversation history" in note
