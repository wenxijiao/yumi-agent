...
from kumi.core.features.stt.router import stt_transcribe_endpoint
...
def test_ensure_whisper_weights_cached_triggers_model_download(monkeypatch, tmp_path, model, repo_id, fw_id):
    ...
    monkeypatch.setattr("kumi.core.features.stt.whisper_provider._whisper_loads_from_disk", lambda **kwargs: False)
...
def test_ensure_whisper_weights_cached_skips_when_local(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("kumi.core.features.stt.whisper_provider._whisper_loads_from_disk", lambda **kwargs: True)
