"""Cover the YUMI_*/TELEGRAM_*/LINE_* environment overrides in load_model_config.
...
def test_chat_and_embedding_overrides(isolated_config, monkeypatch):
    monkeypatch.setenv("YUMI_CHAT_PROVIDER", "openai")
    monkeypatch.setenv("YUMI_CHAT_MODEL", "gpt-x")
    monkeypatch.setenv("YUMI_EMBEDDING_PROVIDER", "gemini")
    monkeypatch.setenv("YUMI_EMBED_MODEL", "embed-1")
...
def test_memory_limit_invalid_value_is_ignored(isolated_config, monkeypatch):
    ...
    monkeypatch.setenv("YUMI_MEMORY_MAX_RECENT", "not-a-number")
    cfg = load_model_config()
...
def test_env_bool_parsing(isolated_config, monkeypatch, raw, expected):
    monkeypatch.setenv("YUMI_CHAT_APPEND_CURRENT_TIME", raw)
...
def test_edge_retrieval_limit_clamped(isolated_config, monkeypatch):
    monkeypatch.setenv("YUMI_EDGE_TOOLS_RETRIEVAL_LIMIT", "500")  # clamp to 200
