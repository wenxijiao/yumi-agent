...
def test_model_config_public_dict_includes_key_flags(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "grok_api_key_saved" in d
    assert "grok_api_key_effective" in d
    assert "openai_base_url" in d
    assert "deepseek_base_url" in d
    assert "grok_base_url" in d
...
def test_create_provider_grok_wraps_openai_provider():
    pytest.importorskip("openai")
    from yumi.core.platform.providers import create_provider
    from yumi.core.platform.providers.openai_provider import OpenAIProvider
...
def test_put_config_model_rejects_grok_embedding_provider(monkeypatch, tmp_path: Path) -> None:
    p = _patch_config_path(monkeypatch, tmp_path)
    p.write_text(
        json.dumps(
            {
    ...
    async def _run():
        with pytest.raises(HTTPException) as ei:
            await update_model_config_endpoint(ModelConfigUpdateRequest(embedding_provider="grok"))
        return ei.value
