...
git clone https://github.com/<your-user>/kumi-agent.git
cd Kumi
# If you cloned into a different folder name (e.g. kumi-agent), use that path instead of `Kumi`.
python -m venv .venv
...
CI (see [.github/workflows/ci.yml](.github/workflows/ci.yml)) runs on each push and PR: Python (`compileall`, `ruff` check + format check, `pytest`, `pytest --cov=kumi.core.api`, `pyright`), plus TypeScript, C++, Go, Swift, Java, Rust, Kotlin, Dart, and C# SDK builds.

The canonical upstream for issues and PRs is [github.com/wenxijiao/kumi-agent](https://github.com/wenxijiao/kumi-agent) (use your fork’s `git clone` URL locally as above).

## Releases (PyPI & npm)

Maintainers can publish **after** bumping versions in [`pyproject.toml`](pyproject.toml) (Python) and [`kumi/sdk/typescript/package.json`](kumi/sdk/typescript/package.json) (TypeScript) to match the git tag.
...
2. Configure repository **Secrets**:
   - `PYPI_API_TOKEN` — [PyPI API token](https://pypi.org/help/#apitoken) for the `kumi-agent` project.
   - `NPM_TOKEN` — [npm automation token](https://docs.npmjs.com/creating-and-viewing-access-tokens) with publish rights for `kumi-sdk`.
...
- Run `ruff check kumi tests` and `ruff format kumi tests` if configured (see `.pre-commit-config.yaml`).

## Compatibility expectations

- Treat the `kumi` CLI, documented HTTP API, and generated SDK templates as the main user-facing surfaces.
- Avoid adding undocumented dependencies on internal modules such as `kumi.core.*` unless the change is clearly marked as internal-only.
...
- Build artifacts under `kumi/ui/.web/`, `kumi/sdk/**/node_modules/`, `kumi/sdk/cpp/build/`, etc. (see [.gitignore](.gitignore))
...
- **Unreal Engine (UE5)** modules require a local UE installation; CI does not compile UE5. See [kumi/sdk/README.md](kumi/sdk/README.md).
