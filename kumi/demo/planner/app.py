"""Kumi Planner — schedule / memo demo for ``kumi --demo``.
...
Process wiring: call ``init_kumi()`` from ``__main__.py`` (after ``Tk()``), then
construct ``PlannerGUI(root, agent)`` — same idea as ``kumi_tools`` / other languages.
"""
...
class PlannerGUI:
    ...
    def __init__(self, root, agent) -> None:
        """Build the planner UI on an existing ``Tk`` root and connected ``KumiAgent``.

        Call ``init_kumi()`` from the process entrypoint (e.g. ``__main__.py``) *after* creating
        the root window, matching other Kumi edge templates and keeping SDK init out of app code.
        """
