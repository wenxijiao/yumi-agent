...
const _logPrefix = '[Kumi]';
const _toolConfirmationFile = '.kumi_tool_confirmation.json';
...
class KumiAgent {
  ...
  KumiAgent(AgentOptions opts) {
    ...
    if (directRelay.isNotEmpty && directToken.isNotEmpty) {
      setKumiEnv('KUMI_RELAY_URL', directRelay);
      setKumiEnv('KUMI_ACCESS_TOKEN', directToken);
    }
  ...
  String _confirmationPath() {
    final o = kumiEnv('KUMI_TOOL_CONFIRMATION_PATH');
...
String _resolveEnvPath(String? explicit) {
  ...
  final a = File('$cwd/kumi_tools/.env');
