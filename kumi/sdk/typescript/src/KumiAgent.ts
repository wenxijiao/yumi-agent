...
import { getEnv, getKumiEnv, isNode } from "./runtime";

const LOG_PREFIX = "[Kumi]";
const TOOL_CONFIRMATION_FILENAME = ".kumi_tool_confirmation.json";
...
export interface KumiAgentOptions {
...
interface KumiWsSession {
  send(data: string): void;
  close(): void;
}

function defaultEdgeName(opts: KumiAgentOptions): string {
...
type WsHandlers = {
  /** Called with the active session so registration can send before `KumiAgent` field is set. */
  onOpen: (session: KumiWsSession) => void;
...
async function openIsomorphicWebSocket(
  ...
  try {
    ...
    throw new Error(
      `Kumi SDK: no WebSocket implementation (browser WebSocket or Node package "ws"). ${String(e)}`
    );
  }
}

/** Load `.env` into `process.env` on Node only (dynamic `fs` — no top-level import). */
async function loadDotEnvFileIfNode(opts: KumiAgentOptions): Promise<{
  ...
  try {
    ...
    if (opts.envPath) {
      ...
      const kumiToolsEnv = path.join(cwd, "kumi_tools", ".env");
      const rootEnv = path.join(cwd, ".env");
      envFile = fs.existsSync(kumiToolsEnv) ? kumiToolsEnv : rootEnv;
    }
...
export class KumiAgent {
  private readonly opts: KumiAgentOptions;
  ...
  private wsSession: KumiWsSession | null = null;
  ...
  constructor(opts: KumiAgentOptions = {}) {
    this.opts = opts;
    this.connectionCode =
      opts.connectionCode ??
      getEnv().KUMI_CONNECTION_CODE ??
      getEnv().BRAIN_URL ??
      undefined;
  ...
  private confirmationPolicyPath(): string {
    const override = (getEnv().KUMI_TOOL_CONFIRMATION_PATH ?? "").trim();
    if (override && isNode()) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
  ...
  private refreshFromEnvAfterDotEnv(): void {
    this.connectionCode =
      ...
      getKumiEnv().KUMI_CONNECTION_CODE ??
      getKumiEnv().BRAIN_URL ??
      undefined;
