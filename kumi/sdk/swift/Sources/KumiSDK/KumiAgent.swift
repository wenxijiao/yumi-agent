...
private let logPrefix = "[Kumi]"
private let confirmationFilename = ".kumi_tool_confirmation.json"
...
/// Embeddable Kumi edge client for Swift.
///
/// The usual approach on iOS is to pass ``connectionCode`` and ``edgeName`` from your app code
/// (e.g. constants in ``KumiSetup.swift``). Bundle resources, scheme env vars, and ``.env``-style
...
/// import KumiSDK
///
/// let agent = KumiAgent(connectionCode: "kumi-lan_...", edgeName: "My iPhone")
///
/// agent.register(
///     name: "set_light",
...
/// ```
public final class KumiAgent: @unchecked Sendable {
    ...
    public init(
        ...
        self.connectionCode = connectionCode ?? env("KUMI_CONNECTION_CODE") ?? env("BRAIN_URL")
        self.edgeName = edgeName ?? env("EDGE_NAME") ?? ProcessInfo.processInfo.hostName
        self.policyBaseDir = resolved.policyBaseDir

        var merged: [String: String] = [:]
        for key in ["KUMI_RELAY_URL", "KUMI_ACCESS_TOKEN", "KUMI_CONNECTION_CODE", "BRAIN_URL", "EDGE_NAME"] {
            if let v = env(key) { merged[key] = v }
        }
        self.envVars = merged
    }

    /// Where we found key=value config and where to persist ``.kumi_tool_confirmation.json``.
    ...
    private static func resolveEnvConfiguration(explicitPath: String?) -> ResolvedEnvConfiguration {
        ...
        if let explicitPath, !explicitPath.isEmpty {
            ...
            if fm.fileExists(atPath: url.path) {
                ...
                if isPathInsideMainBundle(url.path) {
                    policy = writableKumiDataDirectory()
                } else {
        ...
        if let url = findEnvInMainBundle() {
            return ResolvedEnvConfiguration(
                envFileURL: url,
                policyBaseDir: writableKumiDataDirectory()
            )
        }
    ...
    private static func writableKumiDataDirectory() -> String {
        let fm = FileManager.default
        guard let base = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            return NSTemporaryDirectory()
        }
        let dir = base.appendingPathComponent("Kumi", isDirectory: true)
    ...
    private static func findEnvByWalkingUp(from start: String) -> URL? {
        ...
        func candidate(in dir: String) -> URL? {
            let mt = (dir as NSString).appendingPathComponent("kumi_tools/.env")
    ...
    private var confirmationPolicyPath: String {
        if let override = ProcessInfo.processInfo.environment["KUMI_TOOL_CONFIRMATION_PATH"],
