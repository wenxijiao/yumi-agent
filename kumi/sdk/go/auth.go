...
const (
	tokenPrefix    = "kumi_"
	lanTokenPrefix = "kumi-lan_"
)

var legacyLanPrefixes = []string{"ml1_", "kumi_lan_"}
...
// DecodeLanCode parses a kumi-lan_ token into host and port.
func DecodeLanCode(token string) (host string, port int, err error) {
	...
	if strings.HasPrefix(token, lanTokenPrefix) {
		...
		if !found {
			return "", 0, fmt.Errorf("invalid Kumi LAN code prefix")
		}
	}
...
// DecodeCredential parses a kumi_ relay token into its JSON payload.
func DecodeCredential(token string) (map[string]interface{}, error) {
	if !strings.HasPrefix(token, tokenPrefix) {
		return nil, fmt.Errorf("invalid Kumi credential prefix")
	}
