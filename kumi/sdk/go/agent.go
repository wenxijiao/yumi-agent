...
const (
	logPrefix                  = "[Kumi]"
	toolConfirmationFilename   = ".kumi_tool_confirmation.json"
)
...
// KumiAgent is the Go edge client for Kumi.
type KumiAgent struct {
...
// NewAgent creates a new KumiAgent with the given options.
func NewAgent(opts AgentOptions) *KumiAgent {
	var envFile string
	if opts.EnvPath != "" {
		envFile = opts.EnvPath
	} else {
		cwd, _ := os.Getwd()
		kumiToolsEnv := filepath.Join(cwd, "kumi_tools", ".env")
		rootEnv := filepath.Join(cwd, ".env")
		if fileExists(kumiToolsEnv) {
			envFile = kumiToolsEnv
		} else {
			envFile = rootEnv
		}
	}
	...
	if connectionCode == "" {
		connectionCode = os.Getenv("KUMI_CONNECTION_CODE")
	}
	if connectionCode == "" {
...
func (a *KumiAgent) Register(opts RegisterOptions) {
...
func (a *KumiAgent) RunInBackground() {
	a.mu.Lock()
	if len(a.tools) == 0 {
...
func (a *KumiAgent) runSession(wsURL, accessToken string) error {
...
func (a *KumiAgent) handleToolCall(ws *websocket.Conn, msg map[string]interface{}) {
...
func (a *KumiAgent) confirmationPolicyPath() string {
	override := strings.TrimSpace(os.Getenv("KUMI_TOOL_CONFIRMATION_PATH"))
	if override != "" {
		return override
	}
...
func (a *KumiAgent) loadConfirmationPolicy() map[string]interface{} {
...
func (a *KumiAgent) saveConfirmationPolicy(data map[string]interface{}) {
	path := a.confirmationPolicyPath()
...
func (a *KumiAgent) handlePersistPolicy(msg map[string]interface{}) {
