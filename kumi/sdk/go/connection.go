...
func ResolveConnection(code, edgeName string) (*ConnectionConfig, error) {
	relayURL := os.Getenv("KUMI_RELAY_URL")
	accessToken := os.Getenv("KUMI_ACCESS_TOKEN")
	...
	if isRelayToken(code) {
		...
		os.Setenv("KUMI_RELAY_URL", profile.RelayURL)
		os.Setenv("KUMI_ACCESS_TOKEN", profile.AccessToken)
		return &ConnectionConfig{
			Mode:        "relay",
