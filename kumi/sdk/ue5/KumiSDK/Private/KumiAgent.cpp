...
static const FString LogPrefix = TEXT("[Kumi]");
static const FString ConfirmationFilename = TEXT(".kumi_tool_confirmation.json");

// ── FKumiToolArguments ──

FString FKumiToolArguments::GetString(const FString& Key, const FString& Default) const
{
...
int32 FKumiToolArguments::GetInt(const FString& Key, int32 Default) const
{
    if (!Raw.IsValid()) return Default;
    int32 Value;
...
double FKumiToolArguments::GetNumber(const FString& Key, double Default) const
{
    if (!Raw.IsValid()) return Default;
    double Value;
    return Raw->TryGetNumberField(Key, Value) ? Value : Default;
...
bool FKumiToolArguments::GetBool(const FString& Key, bool Default) const
{
    if (!Raw.IsValid()) return Default;
    bool Value;
...
void FKumiAgent::LoadEnvFile(const FString& Path)
{
    FString Content;
...
)
{
    ...
    else
    {
        ...
        FString MtEnv = FPaths::Combine(ProjectDir, TEXT("kumi_tools"), TEXT(".env"));
    ...
    {
        ConnectionCode = FPlatformMisc::GetEnvironmentVariable(TEXT("KUMI_CONNECTION_CODE"));
...
TSharedPtr<FJsonObject> FKumiAgent::BuildToolSchema(const FKumiRegisterOptions& Opts) const
{
...
void FKumiAgent::RegisterTool(FKumiRegisterOptions Opts)
{
...
void FKumiAgent::RunInBackground()
{
    if (Tools.Num() == 0)
    {
    ...
    KumiConnection::ResolveConnection(ConnectionCode, EdgeName,
        [this](TOptional<KumiConnection::FConnectionConfig> Config)
        {
            if (!Config.IsSet())
            {
            ...
            {
                FPlatformMisc::SetEnvironmentVar(TEXT("KUMI_ACCESS_TOKEN"), *Config->AccessToken);
            }
...
FString FKumiAgent::ConfirmationPolicyPath() const
{
    FString Override = FPlatformMisc::GetEnvironmentVariable(TEXT("KUMI_TOOL_CONFIRMATION_PATH"));
    if (!Override.IsEmpty())
        return Override;
    return FPaths::Combine(PolicyBaseDir, ConfirmationFilename);
}

TSharedPtr<FJsonObject> FKumiAgent::LoadConfirmationPolicy() const
{
...
void FKumiAgent::SaveConfirmationPolicy(TSharedPtr<FJsonObject> Data) const
{
    FString Path = ConfirmationPolicyPath();
    FString Output;
...
void FKumiAgent::ConnectToServer()
{
...
void FKumiAgent::OnConnected()
{
    ...
    FString AccessToken = FPlatformMisc::GetEnvironmentVariable(TEXT("KUMI_ACCESS_TOKEN"));
...
void FKumiAgent::OnMessage(const FString& Message)
{
...
void FKumiAgent::HandleToolCall(TSharedPtr<FJsonObject> Msg)
{
    ...
    {
        FKumiToolArguments Args(Arguments);
...
void FKumiAgent::OnClosed(int32 StatusCode, const FString& Reason, bool bWasClean)
{
    WebSocket = nullptr;
    if (!bStopRequested)
    {
...
void FKumiAgent::OnError(const FString& Error)
{
...
void FKumiAgent::ScheduleReconnect()
{
