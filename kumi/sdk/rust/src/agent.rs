...
const LOG_PREFIX: &str = "[Kumi]";
const TOOL_CONFIRMATION_FILENAME: &str = ".kumi_tool_confirmation.json";
...
impl KumiAgent {
    pub fn new(opts: AgentOptions) -> Self {
        ...
        if connection_code.is_empty() {
            connection_code = std::env::var("KUMI_CONNECTION_CODE").unwrap_or_default();
        }
        if connection_code.is_empty() {
        ...
        if edge_name.is_empty() {
            edge_name = hostname::get()
                ...
                .unwrap_or_else(|_| "kumi-edge".into());
        }
...
impl Inner {
    ...
    fn confirmation_policy_path(&self) -> String {
        if let Ok(p) = std::env::var("KUMI_TOOL_CONFIRMATION_PATH") {
            let p = p.trim();
            if !p.is_empty() {
                return p.to_string();
            }
...
fn resolve_env_path(explicit: Option<String>) -> String {
    ...
    let kumi_tools = cwd.join("kumi_tools").join(".env");
    let root = cwd.join(".env");
    if kumi_tools.is_file() {
        kumi_tools.to_string_lossy().to_string()
    } else {
