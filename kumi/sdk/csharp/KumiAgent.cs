...
/// <summary>
/// Kumi edge agent for C#. Connects to the Kumi server over WebSocket,
/// registers tools, and dispatches incoming tool calls to handlers.
/// </summary>
/// <example>
/// <code>
/// var agent = new KumiAgent("kumi-lan_...", "My C# App");
...
/// </code>
/// </example>
public sealed class KumiAgent : IDisposable
{
    private const string LogPrefix = "[Kumi]";
    private const string ToolConfirmationFilename = ".kumi_tool_confirmation.json";
    ...
    /// </summary>
    public KumiAgent(string? connectionCode, string? edgeName, string? envPath = null)
    {
        string envFile;
        ...
        else
        {
            ...
            var kumiToolsEnv = Path.Combine(cwd, "kumi_tools", ".env");
            var rootEnv = Path.Combine(cwd, ".env");
            envFile = File.Exists(kumiToolsEnv) ? kumiToolsEnv : rootEnv;
        }
    ...
    {
        var overridePath = EnvParser.GetEnv("KUMI_TOOL_CONFIRMATION_PATH");
