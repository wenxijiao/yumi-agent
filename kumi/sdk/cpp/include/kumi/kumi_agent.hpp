/**
 * @file kumi_agent.hpp
 * @brief Header-only Kumi Edge SDK — core logic + pluggable transport.
 *
 * ## Batteries included, but pluggable
 * - **IKumiTransport** — abstract WebSocket surface; inject for UE5 / custom stacks.
 * - **DefaultTransport** — when `ixwebsocket/IXWebSocket.h` is on the include path
 *   (and you link IXWebSocket), the default ctor uses it. Define **KUMI_SDK_NO_IXWEBSOCKET**
 ...
 *
 * ## Constructors
 * - `KumiAgent(std::string code)` — creates `DefaultTransport` internally (requires IXWebSocket).
 * - `KumiAgent(std::string code, std::shared_ptr<IKumiTransport> transport)` — custom transport.
 *
 * C++17 or later.
 */
#pragma once
...
#if !defined(KUMI_SDK_NO_IXWEBSOCKET)
#if defined(__has_include)
#if __has_include(<ixwebsocket/IXWebSocket.h>)
#include <ixwebsocket/IXWebSocket.h>
#define KUMI_SDK_IXWEBSOCKET_AVAILABLE 1
#endif
#endif
#endif
 ...
 */
class IKumiTransport {
public:
    virtual ~IKumiTransport() = default;
...
#if defined(KUMI_SDK_IXWEBSOCKET_AVAILABLE)
 ...
 */
class DefaultTransport final : public IKumiTransport {
public:
 ...
 * Use `KumiAgent(code, yourTransport)` with a custom **IKumiTransport**.
 */
class DefaultTransport final : public IKumiTransport {
public:
    ...
    void start() override {
        throw std::runtime_error(
            "Kumi DefaultTransport: IXWebSocket not available at compile time. "
            "Add ixwebsocket to your include path, remove KUMI_SDK_NO_IXWEBSOCKET, "
            "link ixwebsocket, or use KumiAgent(code, std::shared_ptr<IKumiTransport>)."
        );
    }
    void stop() override {}
...
#endif // KUMI_SDK_IXWEBSOCKET_AVAILABLE
...
inline const std::string TOKEN_PREFIX = "kumi_";
inline const std::string LAN_TOKEN_PREFIX = "kumi-lan_";
inline const char* LEGACY_LAN_PREFIXES[] = {"ml1_", "kumi_lan_"};
...
inline LanCodeResult decodeLanCode(const std::string& token) {
    ...
    if (token.compare(0, LAN_TOKEN_PREFIX.size(), LAN_TOKEN_PREFIX) == 0) {
        ...
        if (!matched) {
            throw std::runtime_error("Invalid Kumi LAN code prefix.");
        }
    }
...
inline nlohmann::json decodeCredential(const std::string& token) {
    if (token.compare(0, TOKEN_PREFIX.size(), TOKEN_PREFIX) != 0) {
        throw std::runtime_error("Invalid Kumi credential prefix.");
    }
...
inline std::string httpPost(const std::string& url, const std::string& body, int timeoutSec = 15) {
    ...
    if (useTls) {
        throw std::runtime_error(
            ...
            "Set KUMI_RELAY_URL and KUMI_ACCESS_TOKEN, or use an HTTP relay URL."
        );
    }
...
inline ConnectionConfig resolveConnection(const std::string& code, const std::string& edgeName) {
    std::string relayUrl = getenvStr("KUMI_RELAY_URL");
    std::string accessToken = getenvStr("KUMI_ACCESS_TOKEN");
...
#ifdef _WIN32
        _putenv_s("KUMI_RELAY_URL", profile.relayUrl.c_str());
        _putenv_s("KUMI_ACCESS_TOKEN", profile.accessToken.c_str());
#else
        setenv("KUMI_RELAY_URL", profile.relayUrl.c_str(), 1);
        setenv("KUMI_ACCESS_TOKEN", profile.accessToken.c_str(), 1);
#endif
...
public:
     ...
     */
    explicit KumiAgent(std::string connectionCode)
        : KumiAgent(std::move(connectionCode), std::make_shared<DefaultTransport>()) {}

    /**
     * Default transport + explicit edge name (empty *edgeName* → same as single-arg ctor).
     */
    KumiAgent(std::string connectionCode, std::string edgeName)
        : KumiAgent(
     ...
     */
    KumiAgent(std::string connectionCode, std::string edgeName, std::string envPath)
        : KumiAgent(
              std::move(connectionCode),
              std::move(edgeName),
              std::move(envPath),
    ...
    /**
     * Advanced path: inject **IKumiTransport** (UE5, corporate proxy, tests, …).
     */
    KumiAgent(std::string connectionCode, std::shared_ptr<IKumiTransport> customTransport)
        : transport_(std::move(customTransport)),
          connectionCode_(std::move(connectionCode))
    {
        if (!transport_) {
            throw std::invalid_argument("KumiAgent: customTransport must not be null");
        }
        initFromEnv("", nullptr);
    }
    ...
    KumiAgent(
        ...
        std::string edgeName,
        std::string envPath,
        std::shared_ptr<IKumiTransport> customTransport
    )
        : transport_(std::move(customTransport)),
          connectionCode_(std::move(connectionCode))
    {
    ...
    KumiAgent(const KumiAgent&) = delete;
    KumiAgent& operator=(const KumiAgent&) = delete;
    ...
    void runInBackground() {
        if (tools_.empty()) {
            std::cout << "[Kumi] Warning: no tools registered." << std::endl;
        }
...
private:
    ...
    void initFromEnv(const std::string& envPath, const std::string* edgeNameFromCaller) {
...
#endif
            std::string mtEnv = cwd + "/kumi_tools/.env";
        ...
        if (connectionCode_.empty()) {
            const char* envCode = std::getenv("KUMI_CONNECTION_CODE");
    ...
    std::string confirmationPolicyPath() const {
        const char* override = std::getenv("KUMI_TOOL_CONFIRMATION_PATH");
        if (override && override[0] != '\0') return std::string(override);
        return policyBaseDir_ + "/.kumi_tool_confirmation.json";
    }
    ...
    void connectLoop() {
        ...
        while (!stopRequested_) {
            ...
            try {
                ...
                std::cerr << "[Kumi] Failed to resolve connection: " << e.what() << std::endl;
                return;
            }
            ...
            transport_->setHandlers(
                [this, &connection]() {
                    ...
                    std::cout << "[Kumi] Connected as [" << edgeName_ << "] with "
            ...
            std::cout << "[Kumi] Connection lost. Reconnecting in " << (totalMs / 1000.0) << "s..." << std::endl;
    ...
    std::shared_ptr<IKumiTransport> transport_;
