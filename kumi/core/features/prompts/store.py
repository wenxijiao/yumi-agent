"""Persisted system / per-session prompts (backed by ~/.kumi/config.json via ModelConfig).
...
from kumi.core.features.prompts.defaults import DEFAULT_SYSTEM_PROMPT

# Note: ``kumi.core.features.config`` re-exports several of the functions below, so
...
def get_session_prompt(session_id: str) -> str | None:
    ...
    from kumi.core.features.config.store import load_saved_model_config
...
def _plugin_sections() -> list[str]:
    ...
    Imports the plugin registry lazily because :mod:`kumi.core.features.config`
