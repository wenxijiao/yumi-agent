"""Prompt context assembly for Kumi memory."""
...
from kumi.core.features.memory.retrieval import HybridRetriever
from kumi.core.features.memory.transcript import (
...
def _format_transcript_message(msg: dict) -> dict:
    ...
    if msg["role"] == "assistant":
        if raw.startswith(KUMI_V1_TOOL_CALLS):
            try:
                data = json.loads(raw[len(KUMI_V1_TOOL_CALLS) :])
    ...
    if msg["role"] == "tool":
        if raw.startswith(KUMI_V1_TOOL_RESULT):
            try:
                data = json.loads(raw[len(KUMI_V1_TOOL_RESULT) :])
