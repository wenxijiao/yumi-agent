"""Local Whisper STT via ``faster-whisper`` (bundled with kumi-agent)."""
...
from kumi.core.features.stt.base import SpeechToTextProvider, SttError
...
def ensure_whisper_weights_cached(*, model: str, model_dir: str | None = None) -> None:
    ...
    progress output (``kumi --setup`` stays quiet on repeat runs).
    ...
    try:
        ...
        raise RuntimeError(
            "faster-whisper is required for STT. Install it with: pip install 'kumi-agent[stt]'"
        ) from exc
    fw_id = _FASTER_WHISPER_MODEL_IDS.get(model_name, model_name)
    repo_id = _FASTER_WHISPER_REPO_IDS.get(model_name)
    if not repo_id:
        raise ValueError(
            f"Internal error: no Hugging Face repo for Whisper model {model_name!r}. Update kumi for this STT size."
        )
    hf_token = get_token()
    ...
    print()
    print("  Optional: put HF_TOKEN in ~/.kumi/.env for Hugging Face rate limits (see docs/CONFIGURATION.md).")
    ...
    def _load_model(self):
        ...
        with self._load_lock:
            ...
            try:
                ...
                raise SttError(
                    "faster-whisper is not importable. Install it with: pip install 'kumi-agent[stt]'"
                ) from exc
