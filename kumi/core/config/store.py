...
from kumi.core.config.paths import CONFIG_PATH, ensure_config_dir
...
def load_model_config() -> ModelConfig:
    ...
    chat_provider = os.getenv("KUMI_CHAT_PROVIDER")
    chat_model = os.getenv("KUMI_CHAT_MODEL")
    embedding_provider = os.getenv("KUMI_EMBEDDING_PROVIDER")
    embedding_model = os.getenv("KUMI_EMBED_MODEL")
    ...
    mem_recent = os.getenv("KUMI_MEMORY_MAX_RECENT")
    if mem_recent:
        try:
            ...
            pass
    mem_related = os.getenv("KUMI_MEMORY_MAX_RELATED")
    if mem_related:
        try:
    ...
    config.chat_append_current_time = _env_bool("KUMI_CHAT_APPEND_CURRENT_TIME", config.chat_append_current_time)
    config.chat_append_tool_use_instruction = _env_bool(
        "KUMI_CHAT_APPEND_TOOL_INSTRUCTION", config.chat_append_tool_use_instruction
    )
    config.edge_tools_enable_dynamic_routing = _env_bool(
        "KUMI_EDGE_TOOLS_DYNAMIC_ROUTING", config.edge_tools_enable_dynamic_routing
    )
    mode_from_env = False
    pm_env = os.getenv("KUMI_PROACTIVE_MODE")
    ...
    enabled_env_set = os.getenv("KUMI_PROACTIVE_ENABLED")
    if enabled_env_set is not None and str(enabled_env_set).strip():
        config.proactive_enabled = _env_bool("KUMI_PROACTIVE_ENABLED", config.proactive_enabled)
        if not mode_from_env:
            config.proactive_mode = "smart" if config.proactive_enabled else "off"
    else:
        config.proactive_enabled = _env_bool("KUMI_PROACTIVE_ENABLED", config.proactive_enabled)

    edge_limit = os.getenv("KUMI_EDGE_TOOLS_RETRIEVAL_LIMIT")
    if edge_limit:
        try:
    ...
    proactive_channels = os.getenv("KUMI_PROACTIVE_CHANNELS")
    ...
    proactive_sessions = os.getenv("KUMI_PROACTIVE_SESSION_IDS")
    ...
    if proactive_interval and proactive_interval.strip():
        try:
            ...
            pass
    proactive_idle = os.getenv("KUMI_PROACTIVE_MIN_IDLE_MINUTES")
    if proactive_idle and proactive_idle.strip():
        try:
            config.proactive_min_idle_minutes = max(1, min(10080, int(proactive_idle.strip())))
        except ValueError:
            pass
    proactive_escalation = os.getenv("KUMI_PROACTIVE_UNREPLIED_ESCALATION_MINUTES")
    if proactive_escalation and proactive_escalation.strip():
        try:
            ...
            pass
    proactive_profile = os.getenv("KUMI_PROACTIVE_PROFILE")
    if proactive_profile and proactive_profile.strip():
        config.proactive_profile = proactive_profile.strip()
    proactive_profile_prompt = os.getenv("KUMI_PROACTIVE_PROFILE_PROMPT")
    ...
    proactive_tone = os.getenv("KUMI_PROACTIVE_TONE_INTENSITY")
    if proactive_tone and proactive_tone.strip():
        config.proactive_tone_intensity = proactive_tone.strip()
    proactive_smart_naturalness = os.getenv("KUMI_PROACTIVE_SMART_NATURALNESS")
    ...
    proactive_smart_max_followups = os.getenv("KUMI_PROACTIVE_SMART_MAX_UNREPLIED_FOLLOWUPS")
    if proactive_smart_max_followups and proactive_smart_max_followups.strip():
        try:
            ...
            pass
    local_tz = os.getenv("KUMI_LOCAL_TIMEZONE")
    if local_tz and local_tz.strip():
        config.local_timezone = local_tz.strip()
    else:
        legacy_proactive_tz = os.getenv("KUMI_PROACTIVE_QUIET_HOURS_TIMEZONE")
        if legacy_proactive_tz and legacy_proactive_tz.strip():
            config.local_timezone = legacy_proactive_tz.strip()
    proactive_jitter = os.getenv("KUMI_PROACTIVE_CHECK_INTERVAL_JITTER_RATIO")
    if proactive_jitter and proactive_jitter.strip():
        try:
            ...
            pass
    proactive_esc_jitter = os.getenv("KUMI_PROACTIVE_UNREPLIED_ESCALATION_JITTER_RATIO")
    if proactive_esc_jitter and proactive_esc_jitter.strip():
        try:
            ...
            pass
    proactive_checkin_p = os.getenv("KUMI_PROACTIVE_CHECK_IN_PROBABILITY")
    ...
    proactive_sched_times = os.getenv("KUMI_PROACTIVE_SCHEDULE_TIMES")
    ...
    if proactive_sched_interval and proactive_sched_interval.strip():
        try:
            ...
            pass
    sched_idle = os.getenv("KUMI_PROACTIVE_SCHEDULE_REQUIRE_IDLE")
...
def ensure_full_model_config_file() -> ModelConfig:
    """Write ~/.kumi/config.json with every known config key and current saved values."""
    config = load_saved_model_config()
