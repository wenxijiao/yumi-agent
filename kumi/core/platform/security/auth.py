...
The OSS Kumi server only deals in LAN access codes (host + optional HMAC).
All ``KumiCredential`` / relay access-token / refresh-token machinery has
...
LAN_TOKEN_PREFIX = "kumi-lan_"
_LEGACY_LAN_PREFIXES = ("ml1_", "kumi_lan_")


class KumiLanCode(BaseModel):
    version: int = 1
    host: str
    port: int = 8000
    expires_at: int = 0
...
def encode_lan_code(code: KumiLanCode, secret: str | None = None) -> str:
    payload_dict = {
        "v": code.version,
        "h": code.host,
        "p": code.port,
...
def decode_lan_code(token: str, secret: str | None = None) -> KumiLanCode:
    ...
    if "h" in data:
        ...
        if not parsed.hostname:
            raise ValueError("Kumi LAN code is missing a valid host.")
        code = KumiLanCode(
    ...
    if code.expires_at and code.expires_at < unix_timestamp():
        raise ValueError("Kumi LAN code has expired.")
    ...
    if secret and "s" in data:
        ...
        if not hmac.compare_digest(expected, data["s"]):
            raise ValueError("Kumi LAN code signature is invalid (tampered?).")
