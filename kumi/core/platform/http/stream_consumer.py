...
duplicated 4× inside ``kumi/line/handlers.py`` alone.
...
* :func:`consume_chat_stream` walks any iterator of typed
  :class:`~kumi.core.platform.http.events.ChatEvent` models *or* plain dicts (parsed
  via :func:`~kumi.core.platform.http.events.parse_chat_event`) and dispatches each
...
from kumi.core.platform.http.events import (
    ErrorEvent,
    TextEvent,
    ThoughtEvent,
...
async def consume_chat_stream(
    ...
    ``stream`` may yield typed :class:`~kumi.core.platform.http.events.ChatEvent`
