...
def _build_tool_schema(
    ...
    *,
    ...
    for param_name, param in sig.parameters.items():
        ...
        if annotation == inspect.Parameter.empty:
            print(
                f"[Kumi Tool Warning] Parameter '{param_name}' in tool '{tool_name}' has no type annotation. Defaulting to string."
            )
...
def register_tool(
    ...
    *,
    ...
    """Register a plain function as a Kumi tool (non-decorator API).

    This mirrors the edge-side ``KumiAgent.register()`` pattern::
