...
from kumi.core.prompts.defaults import DEFAULT_SYSTEM_PROMPT

# Note: ``kumi.core.config`` re-exports several of the functions below, so
...
def get_session_prompt(session_id: str) -> str | None:
    ...
    from kumi.core.config.store import load_saved_model_config
...
def _plugin_sections() -> list[str]:
    ...
    Imports the plugin registry lazily because :mod:`kumi.core.config`
