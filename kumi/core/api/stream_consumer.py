...
* :func:`consume_chat_stream` walks any iterator of typed
  :class:`~kumi.core.api.events.ChatEvent` models *or* plain dicts (parsed
  via :func:`~kumi.core.api.events.parse_chat_event`) and dispatches each
...
async def consume_chat_stream(
    ...
    ``stream`` may yield typed :class:`~kumi.core.api.events.ChatEvent`
