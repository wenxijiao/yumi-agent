...
def parse_telegram_chat_id(session_id: str) -> int | None:
    """Map Kumi session ``tg_<user_id>`` to Telegram private chat_id."""
...
async def send_timer_result_to_telegram(
    ...
    if not token:
        logger.info(
            ...
            "On the machine running `kumi --server`, set TELEGRAM_BOT_TOKEN or "
            "telegram_bot_token in ~/.kumi/config.json (same machine as the API)."
        )
        return
...
async def send_text_to_telegram(session_id: str, text: str, *, prefix: str = "") -> bool:
    ...
    if not token:
        logger.info(
            ...
            "Set TELEGRAM_BOT_TOKEN or telegram_bot_token in ~/.kumi/config.json."
        )
        return False
