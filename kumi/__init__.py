"""Public package marker for Kumi.

Quick-start API::

    import kumi

    kumi.register(my_func, "What this function does")
    kumi.run()

This creates a default :class:`KumiAgent` behind the scenes.  For full
control (custom connection code, edge name, multiple agents) use
:class:`kumi.sdk.KumiAgent` directly.
"""

from __future__ import annotations

__all__ = ["__version__", "KumiAgent", "register", "run", "stop"]

__version__ = "0.2.0"

from kumi.sdk import KumiAgent as KumiAgent

_default_agent: KumiAgent | None = None


def _get_default_agent() -> KumiAgent:
    global _default_agent
    if _default_agent is None:
        _default_agent = KumiAgent()
...
def register(
    ...
    *,
    ...
    Equivalent to ``KumiAgent().register(...)`` but uses a shared
    module-level instance so you can write::

        import kumi
        kumi.register(func, "description")
        kumi.run()
    """
