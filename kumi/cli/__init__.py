...
from kumi.core.features.config import (
    ...
    run_model_setup,  # noqa: F401 — re-exported for `kumi.cli.commands.SetupCommand`
...
from kumi.logging_config import configure_logging

SERVER_URL = os.getenv("KUMI_SERVER_URL", "http://127.0.0.1:8000")
...
def _prompt_telegram_bot_token_if_missing() -> bool:
    """If no token in env/config, prompt on TTY and save to ~/.kumi/config.json."""
...
def _prompt_line_credentials_if_missing() -> bool:
    """If LINE secret/token missing, prompt on TTY and save to ~/.kumi/config.json."""
...
def run_server():
    ...
    _print_banner("Kumi Server", rows, ["Mode: local / LAN (single user)"])
    _print_lan_codes()

    subprocess.run([sys.executable, "-m", "kumi.core.api"], env=_subprocess_env_ensure_platform_tokens())


def run_server_with_telegram() -> None:
    """Start Kumi API in a subprocess, then run the Telegram bot against localhost."""
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "kumi.core.api"],
        env=_subprocess_env_ensure_platform_tokens(),
    )
    local_url = "http://127.0.0.1:8000"
    try:
        if not _wait_for_server_health(local_url, timeout=90):
            print("\n  Kumi server did not become healthy in time.\n")
            return
        os.environ["KUMI_SERVER_URL"] = local_url
        ...
        from kumi.telegram.bot import run_telegram_bot_sync
...
def run_server_with_voice() -> None:
    """Start Kumi API with a microphone wake-word loop attached.

    The lifespan checks ``KUMI_VOICE_ENABLED=1`` to decide whether to spawn
    ...
    _print_banner("Kumi Server + Voice", rows, notes)
    _print_lan_codes()

    env = _subprocess_env_ensure_platform_tokens()
    env["KUMI_VOICE_ENABLED"] = "1"
    env["KUMI_VOICE_OWNER_ID"] = owner
    subprocess.run([sys.executable, "-m", "kumi.core.api"], env=env)
...
def run_server_with_telegram_and_voice() -> None:
    ...
    if not (config.voice_porcupine_access_key or os.getenv("PV_ACCESS_KEY")):
        notes.append("WARNING: PV_ACCESS_KEY missing — voice loop will fail to start.")
    _print_banner("Kumi Server + Telegram + Voice", rows, notes)
    _print_lan_codes()

    env = _subprocess_env_ensure_platform_tokens()
    env["KUMI_VOICE_ENABLED"] = "1"
    env["KUMI_VOICE_OWNER_ID"] = owner
    proc = subprocess.Popen([sys.executable, "-m", "kumi.core.api"], env=env)
    local_url = "http://127.0.0.1:8000"
    try:
        if not _wait_for_server_health(local_url, timeout=90):
            print("\n  Kumi server did not become healthy in time.\n")
...
def run_telegram_standalone() -> None:
    """Run only the Telegram bot; connect to the configured local/LAN Kumi API."""
    if not _prompt_telegram_bot_token_if_missing():
        sys.exit(1)

    env = prepare_client_environment("chat")
    target = env.get("KUMI_SERVER_URL", SERVER_URL)
    _print_banner("Kumi Telegram", [("Backend:", target), ("Mode:", "direct")])

    if not is_server_running(server_health_url(target)):
        print("\n  Kumi server is not running. Start it first with: kumi --server\n")
        sys.exit(1)
...
def run_server_with_line() -> None:
    """Start Kumi API in a subprocess, then run the LINE webhook sidecar."""
    ...
    try:
        ...
        from kumi.line.server import run_line_bot_sync
...
def run_line_standalone() -> None:
    ...
    env = prepare_client_environment("chat")
    target = env.get("KUMI_SERVER_URL", SERVER_URL)
    _print_banner("Kumi LINE", [("Backend:", target), ("Mode:", "direct")])
...
def _reflex_ui_root() -> str:
    """Directory of the Reflex app (``kumi/ui``, contains ``rxconfig.py``)."""
...
def run_ui():
    ...
    if not is_server_running(server_health_url(env.get("KUMI_SERVER_URL"))):
        print("\n  Kumi server is not running. Start it first with: kumi --server\n")
        sys.exit(1)
    ...
    if lan_ip:
        ...
        env["KUMI_UI_API_HOST"] = lan_ip
    rows.append(("Backend:", env.get("KUMI_SERVER_URL", SERVER_URL)))

    _print_banner("Kumi UI", rows)
...
def run_chat():
    ...
    target = env.get("KUMI_SERVER_URL", SERVER_URL)
    _print_banner("Kumi Chat", [("Server:", target), ("Mode:", "direct")])

    subprocess.run([sys.executable, "-m", "kumi.chat"], env=env)
...
def _read_env_connection_code(env_path: str) -> str:
    """Read KUMI_CONNECTION_CODE value from a .env file (empty string if unset)."""
    if not os.path.isfile(env_path):
        return ""
    with open(env_path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line.startswith("KUMI_CONNECTION_CODE="):
...
def run_edge(lang: str | list[str] | None = None):
    ...
    env_path = os.path.join(workspace, "kumi_tools", ".env")
    ...
    if existing_code:
        ...
        code = input("  Enter a connection code (LAN kumi-lan_...), or press Enter to skip: ").strip()
        if code:
            ...
            print("  Skipped. Set KUMI_CONNECTION_CODE in kumi_tools/.env later.")
    ...
    if effective is None or "python" in effective:
        print("  [Python]")
        print("  1. Read kumi_tools/python/README.md")
        print("  2. pip install websockets")
        print("  3. Edit kumi_tools/python/kumi_setup.py")
        print("     Import your functions and register them with agent.register()")
        print("  4. In your main program:")
        print("     from kumi_tools.python.kumi_setup import init_kumi")
        print("     init_kumi()")
        print()

    if effective is None or "swift" in effective:
        print("  [Swift]")
        print("  1. Read kumi_tools/swift/README.md (local package vs same-target)")
        print("  2. Add local package: kumi_tools/swift/KumiSDK (has Package.swift)")
        print("  3. Edit KumiSetup.swift: set kumiConnectionCode / kumiEdgeName, call initKumi()")
        print()

    if effective is None or "typescript" in effective:
        print("  [TypeScript / JavaScript]")
        print("  1. Read kumi_tools/typescript/README.md")
        print("  2. cd kumi_tools/typescript/kumi_sdk && npm install")
        print("  3. Edit kumiSetup.ts: set KUMI_CONNECTION_CODE / KUMI_EDGE_NAME")
        print("  4. In your app:")
        print('     import { initKumi } from "./kumi_tools/typescript/kumiSetup";')
        print("     initKumi();")
        print()

    if effective is None or "cpp" in effective:
        print("  [C / C++]")
        print("  1. Read kumi_tools/cpp/README.md")
        print("  2. In your CMakeLists.txt:")
        print("     add_subdirectory(kumi_tools/cpp/KumiSDK)")
        print("     target_link_libraries(your_app PRIVATE kumi_sdk)")
        print("  3. Edit kumi_setup.cpp: set KUMI_CONNECTION_CODE / KUMI_EDGE_NAME")
        print("  4. Call initKumi() in your main()")
        print()

    if effective is None or "ue5" in effective:
        print("  [Unreal Engine 5]")
        print("  1. Read kumi_tools/ue5/README.md")
        print("  2. Copy KumiSDK/ module into your project's Source/ directory")
        print('  3. Add "KumiSDK" to your .Build.cs PublicDependencyModuleNames')
        print("  4. Edit KumiSetup.cpp: set connection code / edge name")
        print()

    if effective is None or "go" in effective:
        print("  [Go]")
        print("  1. Read kumi_tools/go/README.md")
        print("  2. Add to your go.mod:")
        print("     require kumi_sdk v0.0.0")
        print("     replace kumi_sdk => ./kumi_tools/go/kumi_sdk")
        print("  3. Edit kumi_setup.go: set kumiConnectionCode / kumiEdgeName")
        print("  4. Call InitKumi() in your main()")
        print()

    if effective is None or "java" in effective:
        print("  [Java]")
        print("  1. Read kumi_tools/java/README.md")
        print("  2. cd kumi_tools/java/kumi_sdk && mvn install")
        print("     (or copy io.kumi sources into your project)")
        print("  3. Edit KumiSetup.java: set KUMI_CONNECTION_CODE / KUMI_EDGE_NAME")
        print("  4. Call KumiSetup.initKumi() in your main()")
        print()

    if effective is None or "rust" in effective:
        print("  [Rust]")
        print("  1. Read kumi_tools/rust/README.md")
        print("  2. cd kumi_tools/rust && cargo run")
        print("  3. Edit src/kumi_setup.rs: register tools, set edge name / connection code")
        print()

    if effective is None or "kotlin" in effective:
        print("  [Kotlin]")
        print("  1. Read kumi_tools/kotlin/README.md")
        print("  2. cd kumi_tools/kotlin && gradle run (or ./gradlew run)")
        print("  3. Edit KumiSetup.kt under io.kumi.edge")
        print()

    if effective is None or "dart" in effective:
        print("  [Dart]")
        print("  1. Read kumi_tools/dart/README.md")
        print("  2. cd kumi_tools/dart && dart pub get && dart run")
        print("  3. Edit lib/kumi_setup.dart")
        print()
...
def _write_connection_code(env_path: str, code: str) -> None:
    ...
    if os.path.isfile(env_path):
        with open(env_path, "r", encoding="utf-8") as f:
            content = f.read()
        if "KUMI_CONNECTION_CODE=" in content:
            lines = content.splitlines()
            new_lines = []
            for line in lines:
                if line.startswith("KUMI_CONNECTION_CODE="):
                    new_lines.append(f"KUMI_CONNECTION_CODE={code}")
                else:
                    new_lines.append(line)
            content = "\n".join(new_lines) + "\n"
        else:
            content += f"\nKUMI_CONNECTION_CODE={code}\n"
        with open(env_path, "w", encoding="utf-8") as f:
            f.write(content)
    else:
        with open(env_path, "w", encoding="utf-8") as f:
            f.write(f"KUMI_CONNECTION_CODE={code}\n")
...
def _run_demo():
    _print_banner(
        ...
        notes=[
            "Keep `kumi --server` running in another terminal, or enter a LAN code below.",
            "After the windows open, use `kumi --chat` or `kumi --ui` to control both.",
    ...
    if is_server_running(server_health_url()):
        ...
        code = _prompt_connection_code("Enter a LAN connection code (kumi-lan_...), or press Enter for localhost: ")

    from kumi.demo.launcher import run_demo_suite
...
def run_cleanup() -> None:
    print()
    print("  This will remove Kumi user data from this machine:")
    print("  - ~/.kumi/config.json")
    print("  - ~/.kumi/memory/")
    print()
    ...
    if removed_paths:
        ...
        print("  No Kumi user data was found.")
    print()


def run_cleanup_memory() -> None:
    print()
    print("  This will remove Kumi memory data from this machine:")
    print("  - ~/.kumi/memory/")
    print("  - legacy memory under kumi/core/memories/.lancedb (if present)")
    print()
    ...
    if removed_paths:
        ...
        print("  No Kumi memory data was found.")
    print()
...
def prepare_client_environment(scope: str) -> dict:
    """Return an env dict pointing at a reachable local/LAN Kumi API.

    Single-user / LAN only: enterprise relay / multi-tenant profiles are
    handled by ``kumi-enterprise``.
    """
    env = os.environ.copy()
    configured_server_url = env.get("KUMI_SERVER_URL", SERVER_URL)
    ...
    if saved_code and _is_lan_code(saved_code):
        ...
        if direct_url and is_server_running(server_health_url(direct_url)):
            env["KUMI_SERVER_URL"] = direct_url
            masked = saved_code[:8] + "..." + saved_code[-4:] if len(saved_code) > 12 else saved_code
            print(f"  Using saved LAN code ({masked})")
            return env

    print(f"  No reachable Kumi server at {configured_server_url}.")
    join_code = input("  Paste a Kumi LAN code (kumi-lan_...): ").strip()
    if not join_code:
...
def run_tool_routing_config(args) -> None:
    """Show or update edge tool routing settings in ~/.kumi/config.json."""
...
def run_config_file() -> None:
    """Create/update ~/.kumi/config.json with all known fields and open it."""

    ensure_full_model_config_file()
    print()
    print(f"  Kumi config written to: {CONFIG_PATH}")
...
def _open_path_with_default_app(path: Path) -> bool:
    """Best-effort open using the OS default app without blocking Kumi."""
    try:
...
def main():
    """CLI entry point (`kumi = "kumi.cli:main"` in pyproject.toml).

    Implementation lives in :mod:`kumi.cli.commands` (one ``Command`` class
    per sub-command) and :mod:`kumi.cli.registry`. This function only owns
    ...
    parser = argparse.ArgumentParser(
        description="Kumi command line interface",
        epilog="OSS edition: local / LAN single-user. Multi-tenant + remote relay live in kumi-enterprise.",
    )
