"""Concrete :class:`~kumi.cli.registry.Command` implementations.

Each Command is a thin shell around the existing ``run_*`` helper functions
in :mod:`kumi.cli`. The split exists so adding / removing / re-ordering
...
from kumi.cli.registry import Command, CommandRegistry
...
class ServerCommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument("--server", action="store_true", help="Run the Kumi backend server")

    def matches(self, args):
        return bool(args.server)

    def run(self, args):
        from kumi.cli import (
            run_server,
...
class TelegramStandaloneCommand(Command):
    ...
    def run(self, args):
        from kumi.cli import run_telegram_standalone
...
class LineStandaloneCommand(Command):
    ...
    def register(self, parser, mutex_group):
        parser.add_argument(
            ...
            help=(
                ...
                " use KUMI_LINE_INCORE=1 to mount webhook on core instead)"
            ),
        )
    ...
    def run(self, args):
        from kumi.cli import run_line_standalone
...
class VoiceModifierCommand(Command):
    ...
    def register(self, parser, mutex_group):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "With --server: open a microphone wake-word session (say 'hi kumi'). "
                "Requires pip install kumi-agent[voice,stt] and a Picovoice access key."
            ),
        )
...
class UICommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument("--ui", action="store_true", help="Open the Kumi web UI")

    def matches(self, args):
        return bool(args.ui)

    def run(self, args):
        from kumi.cli import run_ui
...
class EdgeCommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument("--edge", action="store_true", help="Initialize a Kumi Edge workspace")
        parser.add_argument(
            "--lang",
            action="append",
            dest="langs",
    ...
    def run(self, args):
        from kumi.cli import _parse_edge_langs, run_edge
...
class DemoCommand(Command):
    ...
    def run(self, args):
        from kumi.cli import _run_demo
...
class SetupCommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument("--setup", action="store_true", help="Configure Kumi models")

    def matches(self, args):
        return bool(args.setup)

    def run(self, args):
        from kumi.cli import run_model_setup
...
class ConfigCommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument(
            "--config",
            action="store_true",
            help="Create/show the full ~/.kumi/config.json settings file",
        )
    ...
    def run(self, args):
        from kumi.cli import run_config_file
...
class ToolRoutingCommand(Command):
    ...
    def run(self, args):
        from kumi.cli import run_tool_routing_config
...
class CleanupCommand(Command):
    ...
    def register(self, parser, mutex_group):
        mutex_group.add_argument("--cleanup", action="store_true", help="Delete Kumi user data")

    def matches(self, args):
        return bool(args.cleanup)

    def run(self, args):
        from kumi.cli import run_cleanup
...
class CleanupMemoryCommand(Command):
    ...
    def run(self, args):
        from kumi.cli import run_cleanup_memory
