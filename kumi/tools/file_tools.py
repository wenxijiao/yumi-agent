...
_SENSITIVE_DIR_NAMES = {".ssh", ".aws", ".gnupg", ".kumi", ".kube"}
...
def _read_docx(path: Path) -> str:
    try:
        ...
        return "Error: python-docx is not installed. Install it with: pip install 'kumi-agent[files]'"
...
def _read_pdf(path: Path) -> str:
    try:
        ...
        return "Error: PyPDF2 is not installed. Install it with: pip install 'kumi-agent[files]'"
...
def read_file(file_path: str) -> str:
    ...
    if _is_sensitive_path(path):
        return (
            ...
            "Credentials, SSH keys, and Kumi's own config are blocked from the read_file tool."
        )
...
def list_files(directory_path: str, pattern: str = "*") -> str:
    ...
    if _is_sensitive_path(path):
        return (
            ...
            "SSH/AWS/GnuPG and Kumi's own config directories are blocked from the list_files tool."
        )
