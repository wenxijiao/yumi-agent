...
from kumi.ui.ui.timer_listener import start_timer_listener, timer_event_queue
...
class State(rx.State):
    ...
    def _is_relay(self) -> bool:
        return bool(os.getenv("KUMI_RELAY_URL") and os.getenv("KUMI_ACCESS_TOKEN"))

    def _base_url(self) -> str:
        if self._is_relay():
            return os.getenv("KUMI_RELAY_URL", DEFAULT_SERVER_URL).rstrip("/")
        return os.getenv("KUMI_SERVER_URL", DEFAULT_SERVER_URL).rstrip("/")
    ...
    def _headers(self) -> dict:
        ...
        token = (os.getenv("KUMI_USER_ACCESS_TOKEN") or os.getenv("KUMI_ACCESS_TOKEN") or "").strip()
        if token:
    ...
    def export_monitor_traces(self):
        try:
            ...
            if not r.ok:
                return
            return rx.download(data=r.content, filename="kumi_tool_traces.ndjson")
        except Exception:
            pass
    ...
    async def handle_chat_file_upload(self, files: list[rx.UploadFile]):
        ...
        if not saved_paths:
            if transcribed_audio:
                ...
                yield rx.clear_selected_files("kumi_chat_upload")
                yield rx.clear_selected_files("kumi_chat_drop")
                return
    ...
    async def send_message(self):
        ...
        if self.pending_files:
            ...
            if img:
                parts.append(
                    "The following images are saved on the Kumi server and will be inlined for the vision model. "
                    "Please view them and describe what you see:\n" + "\n".join(img)
                )
            if doc:
                parts.append(
                    "The following files are saved on the Kumi server. Use the read_file tool to read each path "
...
def sidebar() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            ...
            rx.heading("Kumi", size="3", weight="bold", color="white", letter_spacing="-0.02em"),
            spacing="2",
            align="center",
            padding_x="16px",
            padding_top="16px",
        ...
        rx.upload.root(
            rx.box(
                rx.box(
                    ...
                    margin_x="auto",
                    width="100%",
                    custom_attrs={"data-kumi-chat-input": ""},
                ),
                padding_x="24px",
                padding_y="12px",
                width="100%",
        ...
        rx.dialog.content(
            ...
            rx.vstack(
                ...
                rx.vstack(
                    ...
                    rx.text(
                        "Saved in ~/.kumi/config.json. Leave blank to keep an existing key.",
                        size="1",
                        color="var(--text-2)",
                    ),
                    rx.vstack(
        ...
        rx.cond(
            ...
            rx.center(
                rx.vstack(
                    ...
                    rx.text("Run kumi --edge on a device to connect", size="1", color="var(--text-3)"),
                    align="center",
                    spacing="1",
                ),
                padding="32px",
    ...
    return rx.vstack(
        ...
        rx.vstack(
            rx.hstack(
                ...
                rx.text("Kumi core (this server)", size="2", weight="medium", color="var(--text-1)"),
...
app.add_page(chat_page, route="/", on_load=State.initialize, title="Kumi")
app.add_page(tools_page, route="/tools", on_load=State.init_tools, title="Kumi · Tools")
app.add_page(monitor_page, route="/monitor", on_load=State.init_monitor, title="Kumi · Monitor")
app.add_page(settings_page, route="/settings", on_load=State.init_settings, title="Kumi · Settings")
