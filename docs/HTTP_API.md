# Kumi HTTP API (core server and Relay)

HTTP integration guide for **any language**. Core implementation lives in the [`kumi/core/api/`](../kumi/core/api/) package. The Relay gateway ships in the separate **`kumi-enterprise`** package (not in this OSS repo); see [UPGRADING_TO_ENTERPRISE.md](UPGRADING_TO_ENTERPRISE.md).

## Basics

| Topic | Notes |
|--------|--------|
| Default local base URL | `http://127.0.0.1:8000` (clients may override with `KUMI_SERVER_URL`) |
...
| **Relay mode** (`KUMI_ENABLE_RELAY`) | Clients use Bearer tokens against the Relay base URL; TLS depends on your deployment | Terminate TLS at a reverse proxy when exposing Relay; rotate join codes and tokens if leaked, and set exact browser origins when serving a web app |
...
Kumi now uses **restricted browser defaults**:

- `KUMI_CORS_ORIGINS` controls which browser origins may call the **core** API.
- `KUMI_CORS_ALLOW_CREDENTIALS` controls whether browsers may send credentials to the **core** API.
- `KUMI_RELAY_CORS_ORIGINS` controls which browser origins may call the **Relay** API.
- `KUMI_RELAY_CORS_ALLOW_CREDENTIALS` controls whether browsers may send credentials to the **Relay** API.

Behavior:

- If unset, both services allow only localhost-style development origins.
- Browser credentials are **off by default**.
- If you set origins to `*`, Kumi forces browser credentials back off because wildcard origins and credentialed requests are incompatible in browsers.

Examples:

```bash
# Allow a production web app to call Relay over HTTPS.
export KUMI_RELAY_CORS_ORIGINS="https://app.example.com"

# Allow a custom browser client to call the core API on a trusted private network.
export KUMI_CORS_ORIGINS="https://dashboard.example.internal"
```
...
```bash
export KUMI_SERVER_URL="${KUMI_SERVER_URL:-http://127.0.0.1:8000}"

curl -sN -X POST "$KUMI_SERVER_URL/chat" \
...
`kumi-agent` (this OSS package) is single-user / LAN. Multi-tenant routes (`/tenancy/*`, `/admin/*`, `/auth/*`, `/relay/*`, `/telegram/link`, `/line/link`, …) are added by the closed-source `kumi-enterprise` plugin via the `kumi.core.plugins` port system. See [UPGRADING_TO_ENTERPRISE.md](UPGRADING_TO_ENTERPRISE.md) for details.
...
| `GET` / `PUT` | `/config/model` | Read/update model, memory, and Edge tool-routing settings; `PUT` may include `edge_tools_enable_dynamic_routing`, `edge_tools_retrieval_limit`, `openai_api_key`, `gemini_api_key`, `claude_api_key`, `deepseek_api_key`, `openai_base_url`, `deepseek_base_url` (non-empty key values are saved to `~/.kumi/config.json`; `GET` never returns raw keys, only `*_saved` / `*_effective` flags and saved base URLs; **`embedding_provider` cannot be `deepseek`**) |
| `GET` / `PUT` / `DELETE` | `/config/session-prompt/{session_id}` | Per-session system prompt override |
| `GET` / `PUT` | `/config/chat-debug` | Per-session chat NDJSON tracing: `GET` returns `enabled` and `trace_path`; `PUT` body `{"session_id":"...", "enabled": true|false}` starts/stops appending trace lines under `KUMI_DEBUG_DIR/chat_trace/`. Each turn logs `llm_provider_request` with the full provider `messages` and `tools` after prompt composition (system prompt, memory, tool results, multimodal parts). Optional `KUMI_CHAT_DEBUG_REDACT_IMAGE_DATA` shrinks logged base64 (see configuration docs). |
...
| Field | Meaning |
|-------|---------|
| `code` | Stable machine-readable identifier (e.g. `KUMI_MISSING_OPENAI_KEY`, `KUMI_OLLAMA_UNAVAILABLE`, `KUMI_UNKNOWN_PROVIDER`, `KUMI_PROVIDER_MODEL_APPLY_FAILED`) |
...
Traces may be mirrored to `~/.kumi/tool_traces.jsonl` on disk (append-only); the in-memory buffer is also bounded.
...
## Relay gateway (`KUMI_ENABLE_RELAY=1`)
...
Obtain `access_token` from `POST /v1/bootstrap` using a join code; see the Relay / bootstrap types in **`kumi-enterprise`** (not shipped in this repository).
...
Run `kumi --server` locally first, and complete `kumi --setup` (or set model-related environment variables). The **`POST /chat`** section above documents the NDJSON line protocol and includes a **curl** example. Interactive exploration: open `/docs` on the running server.

---

## Stability notes

Kumi is still in the **0.x** stage. For HTTP integrations, treat the documented routes in this file and the generated OpenAPI schema as the intended public contract. Internal Python modules and undocumented routes may change between releases.
