"""Persist Yumi Bearer tokens per Telegram user after ``/link`` (local bridge process)."""
...
def chat_connection_config(telegram_user_id: int | None):
    """Like ``resolve_connection_config("chat")`` but attach Yumi user token when linked."""
    import os
