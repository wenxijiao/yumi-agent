"""Yumi Edge workspace initializer.

The WebSocket client logic has moved to :class:`yumi.sdk.YumiAgent`.
This module only provides ``init_workspace()`` which scaffolds the
``yumi_tools/`` directory and ``.env`` file for a new edge project.
"""
...
_STANDALONE_FILES = {
    "env.template": os.path.join("yumi_tools", ".env"),
    "gitignore.template": ".gitignore",
}

_AGENT_GUIDE_DEST = "AGENTS.md"
_TEMPLATE_SUBDIR = "yumi_tools"
...
_SDK_SOURCES = {
    "python": {
        ...
        "dest_subdir": os.path.join("python", "yumi_sdk"),
    },
    "swift": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "swift"),
        "ignore": [".build", ".swiftpm"],
        "dest_subdir": os.path.join("swift", "YumiSDK"),
    },
    "typescript": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "typescript"),
        "ignore": ["node_modules", "dist"],
        "dest_subdir": os.path.join("typescript", "yumi_sdk"),
    },
    "cpp": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "cpp"),
        "ignore": ["build", ".cache"],
        "dest_subdir": os.path.join("cpp", "YumiSDK"),
    },
    "ue5": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "ue5", "YumiSDK"),
        "ignore": ["Intermediate", "Binaries"],
        "dest_subdir": os.path.join("ue5", "YumiSDK"),
    },
    "go": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "go"),
        "ignore": ["vendor"],
        "dest_subdir": os.path.join("go", "yumi_sdk"),
    },
    "java": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "java"),
        "ignore": ["target", ".idea"],
        "dest_subdir": os.path.join("java", "yumi_sdk"),
    },
    "csharp": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "csharp"),
        "ignore": ["bin", "obj"],
        "dest_subdir": os.path.join("csharp", "yumi_sdk"),
    },
    "rust": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "rust"),
        "ignore": ["target", ".cargo"],
        "dest_subdir": os.path.join("rust", "yumi_sdk"),
    },
    "kotlin": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "kotlin", "src", "main", "kotlin", "io", "yumi", "sdk"),
        "ignore": [],
        "dest_subdir": os.path.join("kotlin", "src", "main", "kotlin", "io", "yumi", "sdk"),
    },
    "dart": {
        "mode": "tree",
        "sdk_rel": os.path.join("sdk", "dart"),
        "ignore": [".dart_tool", "build"],
        "dest_subdir": os.path.join("dart", "yumi_sdk"),
    },
}


def _find_sdk_root() -> str:
    """Locate the ``yumi/`` package root (parent of ``sdk/``)."""
...
def _copy_agent_guide(workspace: str, created: list[str]) -> None:
    ...
    try:
        guide = _pkg_resources.files("yumi.sdk") / "AGENTS.md"
...
def init_workspace(
    ...
    """Ensure a Yumi Edge workspace is complete in *target_dir*.
    ...
    if not os.path.isdir(template_dir):
        try:
            ref = _pkg_resources.files("yumi.edge") / "template"
