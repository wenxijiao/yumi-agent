"""Standalone LINE webhook server (topology B: sidecar to core Yumi HTTP API)."""
...
from yumi.core.features.config.line import get_line_bot_port, get_line_channel_secret
...
def build_line_app() -> FastAPI:
    app = FastAPI(title="Yumi LINE webhook")
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "yumi-line-webhook"}
