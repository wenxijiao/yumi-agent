...
export function getYumiEnv(): Record<string, string | undefined> {
  const e = { ...getEnv() };
  if (browserRelay.relayUrl) {
    e.YUMI_RELAY_URL = browserRelay.relayUrl;
    e.YUMI_ACCESS_TOKEN = browserRelay.accessToken;
  }
  return e;
}
...
export function setRelayFromBootstrap(relayUrl: string, accessToken: string): void {
  if (isNode() && typeof process !== "undefined" && process.env) {
    process.env.YUMI_RELAY_URL = relayUrl;
    process.env.YUMI_ACCESS_TOKEN = accessToken;
  }
