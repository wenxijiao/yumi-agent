...
const TOKEN_PREFIX = "yumi_";
const LAN_TOKEN_PREFIX = "yumi-lan_";
const LEGACY_LAN_PREFIXES = ["ml1_", "yumi_lan_"];
...
export function decodeLanCode(token: string): LanCodeResult {
  ...
  if (token.startsWith(LAN_TOKEN_PREFIX)) {
    ...
    if (prefix) {
      ...
      throw new Error("Invalid Yumi LAN code prefix.");
    }
  }
...
export function decodeCredential(token: string): Record<string, unknown> {
  if (!token.startsWith(TOKEN_PREFIX)) {
    throw new Error("Invalid Yumi credential prefix.");
  }
...
export async function bootstrapProfile(
  ...
  if (typeof globalThis.fetch !== "function") {
    throw new Error(
      "Yumi SDK: global fetch is not available. Use Node.js 18+ or a browser, or set YUMI_RELAY_URL + YUMI_ACCESS_TOKEN."
    );
  }
