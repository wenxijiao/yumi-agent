...
let tokenPrefix = "yumi_"
let lanTokenPrefix = "yumi-lan_"
let legacyLanPrefixes = ["ml1_", "yumi_lan_"]

// MARK: - Decoded types

struct YumiCredential {
    let relayURL: String
    let tunnelID: String
    let scopes: [String]
    let expiresAt: Int
...
enum AuthError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidPrefix:       return "Invalid Yumi token prefix."
        case .decodeFailed:        return "Failed to decode Yumi token payload."
...
/// Decode a ``yumi-lan_`` (or legacy prefix) LAN connection code into host + port.
func decodeLanCode(_ token: String) throws -> YumiLanCode {
    let encoded: String
    ...
    return YumiLanCode(host: host, port: port)
}

/// Decode a ``yumi_`` credential token into its fields.
func decodeCredential(_ token: String) throws -> YumiCredential {
