...
{
    FString RelayUrl = GetEnvVar(TEXT("YUMI_RELAY_URL"));
    FString AccessToken = GetEnvVar(TEXT("YUMI_ACCESS_TOKEN"));
    ...
    if (YumiAuth::IsLanCode(Code))
    {
        FString ServerUrl = YumiAuth::ParseLanCode(Code);
...
)
{
    ...
    if (YumiAuth::IsRelayToken(Code))
    {
        YumiAuth::BootstrapProfile(Code, TEXT("edge"), EdgeName,
            [OnResolved](TOptional<YumiAuth::FBootstrapResult> BootstrapResult)
            {
                ...
                FPlatformMisc::SetEnvironmentVar(TEXT("YUMI_RELAY_URL"), *BootstrapResult->RelayUrl);
                FPlatformMisc::SetEnvironmentVar(TEXT("YUMI_ACCESS_TOKEN"), *BootstrapResult->AccessToken);
