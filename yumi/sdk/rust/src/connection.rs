...
pub fn resolve_connection(code: &str, edge_name: &str) -> Result<ConnectionConfig, String> {
    let relay_url = env::var("YUMI_RELAY_URL").unwrap_or_default();
    let access_token = env::var("YUMI_ACCESS_TOKEN").unwrap_or_default();
    ...
    if is_relay_token(code) {
        ...
        env::set_var("YUMI_RELAY_URL", &profile.relay_url);
        env::set_var("YUMI_ACCESS_TOKEN", &profile.access_token);
        return Ok(ConnectionConfig {
            mode: "relay",
