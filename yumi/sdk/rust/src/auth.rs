...
const TOKEN_PREFIX: &str = "yumi_";
const LAN_TOKEN_PREFIX: &str = "yumi-lan_";
const LEGACY_LAN_PREFIXES: [&str; 2] = ["ml1_", "yumi_lan_"];
...
/// Parse a `yumi-lan_...` token into `(host, port)`.
pub fn decode_lan_code(token: &str) -> Result<(String, u16), String> {
    let encoded = if token.starts_with(LAN_TOKEN_PREFIX) {
        ...
        found.ok_or_else(|| "invalid Yumi LAN code prefix".to_string())?
    };
...
fn decode_credential_json(token: &str) -> Result<HashMap<String, Value>, String> {
    if !token.starts_with(TOKEN_PREFIX) {
        return Err("invalid Yumi credential prefix".into());
    }
