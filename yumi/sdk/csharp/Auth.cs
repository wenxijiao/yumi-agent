...
{
    private const string TokenPrefix = "yumi_";
    private const string LanTokenPrefix = "yumi-lan_";
    private static readonly string[] LegacyLanPrefixes = { "ml1_", "yumi_lan_" };
    ...
    public static (string host, int port) DecodeLanCode(string token)
    {
        ...
        if (encoded is null)
            throw new ArgumentException("Invalid Yumi LAN code prefix.");
    ...
    public static JsonElement DecodeCredential(string token)
    {
        if (!token.StartsWith(TokenPrefix))
            throw new ArgumentException("Invalid Yumi credential prefix.");
