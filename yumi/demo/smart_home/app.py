"""Yumi Smart Home Simulator — state and tool functions.
...
are imported by ``smart_home/setup.py`` and registered with the Yumi SDK,
just like a real user would do in their own project.

Run with: ``python -m yumi.demo.smart_home`` or ``yumi --demo``. ``init_yumi()`` is
...
class SmartHomeGUI:
    ...
    def __init__(self, agent=None):
        self._agent = agent
        self._root = tk.Tk()
        self._root.title("Yumi Smart Home Simulator")
    ...
    def _build_ui(self):
        ...
        subtitle = tk.Label(
            r,
            text=("Display only. Use yumi --chat or yumi --ui to control Smart Home and Planner in one conversation."),
            font=("Helvetica", 10),
            bg=COL_BG,
            fg=COL_DIM,
            wraplength=880,
