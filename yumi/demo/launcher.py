"""Launch the Yumi demo suite.

``yumi --demo`` starts two independent Python GUI applications:

- Smart Home
- Planner (schedule)

Each process runs its own ``YumiAgent`` and registers a separate tool host.
"""
...
def run_demo_suite(connection_code: str | None = None) -> None:
    modules = ("yumi.demo.smart_home", "yumi.demo.planner")
    for m in modules:
        try:
            ...
            print("  From your Yumi clone run: pip install -e .")
            print()
