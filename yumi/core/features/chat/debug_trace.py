...
def chat_debug_redact_image_data_urls() -> bool:
    ...
    raw = (os.getenv("YUMI_CHAT_DEBUG_REDACT_IMAGE_DATA") or "").strip().lower()
...
def append_llm_provider_request(
    ...
    *,
    ...
    Set ``YUMI_CHAT_DEBUG_REDACT_IMAGE_DATA=1`` to shorten inline ``data:`` image URLs in the log only.
    """
    try:
