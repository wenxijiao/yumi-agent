...
The implementation lives in ``yumi.core.features.chat.service``. This module
keeps the historical ``generate_chat_events`` / ``clear_session`` API stable
for the three call sites:

* ``yumi.core.features.chat.router`` (the HTTP endpoint),
* ``yumi.core.features.proactive.scheduler`` (timer-fired follow-up turns),
* ``yumi.line.handlers`` (LINE direct-mode bridge).

Internally the orchestrator now yields :class:`~yumi.core.platform.http.events.ChatEvent`
...
:mod:`yumi.core.platform.http.events` and use :func:`parse_chat_event` (or migrate to
the channel-handler pattern in :mod:`yumi.core.platform.http.stream_consumer`).
"""
...
from yumi.core.platform.runtime.accessors import SESSION_LOCKS
...
async def generate_chat_events(
    ...
    *,
    ...
    importing module (``yumi.core.features.chat.router`` for the HTTP layer).
    """
