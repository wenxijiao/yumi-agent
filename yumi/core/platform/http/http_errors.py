"""Structured HTTP error bodies for the Yumi API (FastAPI ``HTTPException`` ``detail`` dicts)."""
...
def provider_not_ready_http(exc: ProviderNotReadyError) -> HTTPException:
    ...
    status = 503 if exc.code == "YUMI_OLLAMA_UNAVAILABLE" else 400
...
def model_apply_failed_http(*, phase: str, exc: Exception) -> HTTPException:
    ...
    return HTTPException(
        status_code=502,
        detail={
            "code": "YUMI_PROVIDER_MODEL_APPLY_FAILED",
