...
def erase_assistant_data(memory, owner, qualify, *, account=False, memories=False):
    ...
    with sqlite.connect() as conn:
        ...
        if "message_index_jobs" in tables:
            conn.execute(
                "DELETE FROM message_index_jobs WHERE event_id NOT IN (SELECT id FROM events WHERE deleted_at IS NULL)"
            )
        if "voice_messages" in tables:
            conn.executemany(
        ...
        # access audit metadata is kept separately by the host application.
