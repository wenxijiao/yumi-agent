...
_DIAGNOSED_ATTR = "_yumi_provider_failure_diagnostic_path"


def debug_dir() -> str:
    """Return the directory used for provider failure diagnostics."""
    override = (os.getenv("YUMI_DEBUG_DIR") or "").strip()
    if override:
        return os.path.expanduser(override)
    return os.path.expanduser("~/.yumi/debug")
...
def write_provider_failure_diagnostic(
    *,
    ...
    (e.g. ``embed`` or ``retry``) for easier scanning in ``~/.yumi/debug/``.
    """
