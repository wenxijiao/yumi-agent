/// One-call "connect this app to a Mirai deployment" helper.
///
/// Any third-party app that wants to expose tools to a Mirai deployment goes
...
///                                                       │
/// App  ──MiraiAgent(connectionCode: access_token)──▶  Mirai
/// ```
///
...
/// the helper takes a [MiraiAppHandshakeAdapter]. The SDK ships a desktop
...
class MiraiAppTokens {
  /// The Mirai access token to pass as ``connectionCode`` to [MiraiAgent].
  /// Required.
  final String accessToken;

  /// Optional identity-provider ID token (e.g. Firebase ID token) that the
  /// portal sometimes returns alongside the Mirai access token. The SDK
  ...
  const MiraiAppTokens({
    required this.accessToken,
    this.idToken,
    this.extra = const {},
  });

  factory MiraiAppTokens.fromCallback(Uri callback) {
    final params = callback.queryParameters;
    final access = (params['access_token'] ?? '').trim();
    if (access.isEmpty) {
      throw const MiraiAppHandshakeException(
  ...
  factory MiraiAppTokens.fromJson(Map<String, dynamic> json) {
...
class MiraiAppHandshakeException implements Exception {
  final String message;
  const MiraiAppHandshakeException(this.message);

  @override
  String toString() => 'MiraiAppHandshakeException: $message';
}

/// Platform glue for opening the portal URL and receiving the callback.
abstract class MiraiAppHandshakeAdapter {
  ...
  Future<void> storeTokens(String appId, MiraiAppTokens tokens);

  /// Load previously stored tokens for [appId], or null if none.
  Future<MiraiAppTokens?> loadStoredTokens(String appId);

  /// Forget any stored tokens for [appId] (called by [MiraiApp.disconnect]).
...
/// in [storageDir] (``~/.mirai/app_tokens/<appId>.json`` by default).
class LoopbackHandshakeAdapter implements MiraiAppHandshakeAdapter {
  ...
  Directory get _resolvedStorageDir {
    ...
    return Directory('$home/.mirai/app_tokens');
  }
  ...
  Future<Uri> openAndAwaitCallback({
    ...
    try {
      ...
      final timer = Timer(callbackTimeout, () {
        if (!completer.isCompleted) {
          completer.completeError(
            const MiraiAppHandshakeException(
      ...
      server.listen((req) async {
        ...
        if (!completer.isCompleted) {
          if (ok) {
            ...
            completer.completeError(
              const MiraiAppHandshakeException('Portal callback state mismatch.'),
            );
          }
        }
      });
      if (onOpenUrl != null) {
        ...
        // ignore: avoid_print
        print('[Mirai] Open this URL to finish connecting:\n${portalUrl.toString()}');
      }
  ...
  @override
  Future<MiraiAppTokens?> loadStoredTokens(String appId) async {
    final f = _tokenFile(appId);
    if (!await f.exists()) return null;
    try {
      final decoded = jsonDecode(await f.readAsString());
      if (decoded is Map<String, dynamic>) {
        return MiraiAppTokens.fromJson(decoded);
      }
    } catch (_) {}
    return null;
  }
...
/// Bind options for [MiraiApp.connect].
class MiraiAppConnectOptions {
...
/// Top-level helper: portal handshake → token persistence → started [MiraiAgent].
class MiraiApp {
  ...
  ///    persist them.
  /// 3. Construct a [MiraiAgent] using the access token as ``connectionCode``
  ...
  /// ```
  static Future<MiraiAgent> connect(
    MiraiAppHandshakeAdapter adapter,
    MiraiAppConnectOptions opts,
  ) async {
    MiraiAppTokens? tokens;
    if (opts.reuseStored) {
  ...
  /// refresh).
  static Future<MiraiAgent> reconnect(
    MiraiAppHandshakeAdapter adapter,
    MiraiAppConnectOptions opts,
  ) async {
  ...
  static Future<MiraiAppTokens> _runHandshake(
    MiraiAppHandshakeAdapter adapter,
    MiraiAppConnectOptions opts,
  ) async {
